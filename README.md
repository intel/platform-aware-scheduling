...
Telemetry Aware Scheduling makes telemetry data available to scheduling and descheduling decisions in Kubernetes. The system, through a user defined policy, enables rule based decisions on pod scheduling based on telemetry data. Policies can be applied on a workload by workload basis - allowing the correct telemetry to be attached to the right pod.
...
## Components
Telemetry Aware Scheduling is made up of two independent components. 
...
The Telemetry Policy Controller consumes Telemetry Policies - a Custom Resource. The resource is created by the controller on startup - if it doesn't exist already.
The controller parses this policy for a deschedule strategy - a list of rules that if violated signal the workload should be descheduled from its current host.
It consumes new Telemetry Policies as they are created, removes them when deleted, and updates them as they are changed.

## Usage
### Quick set up
The deploy folder has all of the yaml files necessary to get Telemetry Aware Scheduling running in a Kubernetes cluster. A couple of additional steps are required to configure the scheduler and metrics endpoints, however.
